...
const { proposer, config } = require('@onflow/fcl');
...
module.exports = class DappLib {
  ...
  // if Uint64, must wrap in parseItn
  //
...
// 
static async setupAllAccounts(data) {
  let config = DappLib.getConfig()
  let result = await Blockchain.post({
    config: config,
    roles: {
  ...
  let result2 = await Blockchain.post({
    config: config,
    roles: {
      proposer: data.signer
    }
  ...
  let result3 = await Blockchain.post({
    config: config,
    roles: {
      proposer: data.signer
    }
  ...
  let resultArray = [
    result.callData.transactionId, 
    result2.callData.transactionId, 
    result3.callData.transactionId
  ]

  return { 
    type: DappLib.DAPP_RESULT_ARRAY,
    label: 'Account creation transactions',
    result: resultArray
}
}
