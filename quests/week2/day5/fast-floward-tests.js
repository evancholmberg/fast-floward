...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`checks that user has all 3 accounts created`, async () => {
            let testData1 = {
                address: config.accounts[3]
            }

            res1 = await DappLib.setupAllAccounts(testData1)

            assert.equal(res1.result.length, 3, 'Correct number of transactions')
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            assert.equal(res1.result, 1, "User's Market Collection should have length of 1")

            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res2.result[0], 1, "The correct itemID exists in the users sale collection")
        })
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            res1 = await DappLib.kibbleGetBalance(testData1)

            // 4) call kibbleGetBalance with testData2 and store the result
            res2 = await DappLib.kibbleGetBalance(testData2)

            // 5) assert the result from step 3) equals 5.0
            assert.equal(res1.result, 5.0, "account 1 has the correct kibble balance")

            // 6) assert the result from step 4) equals 25.0
            assert.equal(res2.result, 25.0, "account 2 has the correct kibble balance")
        })
